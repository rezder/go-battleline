...
func benchServe(watchChCl *WatchChCl, game <-chan *MoveBenchView) {
	watchers := make(map[int]chan<- *MoveBench)
	var initMove *MoveBench
Loop:
	for {
		select {
...
func NewMoveBench(view *MoveBenchView, ini bool) (move *MoveBench) {
	move = new(MoveBench)
