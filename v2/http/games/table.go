...
func tableServe(ids [2]int, playerChs [2]chan<- *MoveView, watchChCl *WatchChCl, resumeGame *bat.Game,
	finishCh chan *FinishTableData, errCh chan<- error) {

	var moveChs [2]chan [2]int
	moveChs[0] = make(chan [2]int)
	moveChs[1] = make(chan [2]int)
	benchCh := make(chan *MoveBenchView, 1)
	go benchServe(watchChCl, benchCh)
	...
	for {
		...
		if !open {
			isSaveMove = true
			move = *NewMoveSave()
		} else if moveix[1] == SMQuit {
			game.Quit(game.Pos.Player)
			move = *NewMoveQuit()
		} else if moveix[1] == SMPass {
			game.Pass()
			move = *NewMovePass()
		} else if moveix[0] > 0 {
...
func creaMove(mover int, move bat.Move, moveCardix int, deltCardix int, pos *bat.GamePos, ids [2]int) (move1 *MoveView, move2 *MoveView, moveBench *MoveBenchView) {
	move1 = new(MoveView)
	move1.Mover = mover == 0
	move1.Move = move
	move1.MoveCardix = moveCardix
	move1.Turn = NewTurn(&pos.Turn, 0)

	move2 = new(MoveView)
	move2.Mover = mover == 1
	move2.Move = move
	move2.MoveCardix = moveCardix
	move2.Turn = NewTurn(&pos.Turn, 1)
...
func initMove(resumeGame *bat.Game, ids [2]int, moveChans [2]chan [2]int) (game *bat.Game, move1 *MoveView,
	move2 *MoveView, moveBench *MoveBenchView) {
	move1 = new(MoveView)
	move2 = new(MoveView)
	moveBench = new(MoveBenchView)
	moveBench.Mover = -1
...
//NewPlayPos creates a new player position.
...
// MoveRedeployView the redeploy move view.
...
// MoveDeserterView the deserter move view.
